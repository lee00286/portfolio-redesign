...
describe('POST /projects', () => {
  ...
  it('returns 400 if project_id is missing', async () => {
    ...
    const res2 = await POST({ json: async () => ({ project_id: '' }) });

    expect(res1.status).toBe(400);
    expect(res1.body.error).toBe('project_id is required.');
    expect(res2.status).toBe(400);
    expect(res2.body.error).toBe('project_id is required.');
  });
  ...
  it('returns 403 if admin is disabled', async () => {
    ...
    const res = await POST({
      json: async () => ({ project_id: 'new-project' })
    });
  ...
  it('returns 409 if project_id already exists', async () => {
    createAdminSupabaseServer.mockReturnValue(supabase);

    supabase.maybeSingle.mockResolvedValueOnce({
      data: { id: '1', project_id: 'new-project' },
      error: null
    });
    ...
    expect(res.body.error).toBe('Project ID already exists');
  });
  ...
  it('returns 500 if supabase create fails', async () => {
    ...
    const res = await POST({
      json: async () => ({ project_id: 'new-project-500', name: 'Test' })
    });
  ...
  it('creates project successfully', async () => {
    const req = {
      json: async () => ({ project_id: 'new-project-3', name: 'Test' })
    };
...
describe('PUT /projects/[id]', () => {
  ...
  it('returns 404 if project does not exist', async () => {
    ...
    expect(res.body.error).toBe('Project data not found');
  });
...
describe('PATCH /projects/[id]', () => {
  ...
  it('returns 404 if project does not exist', async () => {
    ...
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('Archived project data not found');
  });
  ...
  it('restores project successfully if is_active is false', async () => {
    supabase = createSupabaseMock({
      id: '123',
      project_id: 'new-project-123',
      title: '123',
  ...
  it('restores project successfully if is_active is true', async () => {
    supabase = createSupabaseMock({
      id: '124',
      project_id: 'new-project-124',
      title: '124',
      is_active: true,
      deleted_at: null
    });
...
describe('DELETE /projects/[id]', () => {
  ...
  it('soft deletes project successfully if deleted_at is not null', async () => {
    supabase = createSupabaseMock({
      id: '123',
      project_id: 'new-project-123',
      title: 'test 123',
  ...
  it('soft deletes project successfully if is_active is true', async () => {
    supabase = createSupabaseMock({
      id: '123',
      project_id: 'new-project-123',
      title: 'test 123',
  ...
  it('soft deletes project successfully if is_active is false', async () => {
    supabase = createSupabaseMock({
      id: '124',
      project_id: 'new-project-124',
      title: 'test 124',
