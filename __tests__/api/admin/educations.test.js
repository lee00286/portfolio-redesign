...
const { POST } = require('@/app/api/admin/educations/route');
const { PUT, PATCH, DELETE } = require('@/app/api/admin/educations/[id]/route');
...
describe('POST /educations', () => {
  ...
  it('returns 400 if education_id is missing', async () => {
    const res1 = await POST({ json: async () => ({}) });
    const res2 = await POST({ json: async () => ({ education_id: '' }) });

    expect(res1.status).toBe(400);
    expect(res1.body.error).toBe('education_id is required.');
    expect(res2.status).toBe(400);
    expect(res2.body.error).toBe('education_id is required.');
  });
  ...
  it('returns 403 if admin is disabled', async () => {
    ...
    const res = await POST({
      json: async () => ({ education_id: 'new-education' })
    });
  ...
  it('returns 409 if education_id already exists', async () => {
    createAdminSupabaseServer.mockReturnValue(supabase);

    supabase.maybeSingle.mockResolvedValueOnce({
      data: { id: '1', education_id: 'new-education' },
      error: null
    });
    ...
    expect(res.body.error).toBe('Education ID already exists');
  });
  ...
  it('returns 500 if supabase create fails', async () => {
    ...
    const res = await POST({
      json: async () => ({ education_id: 'new-education-500', name: 'Test' })
    });
  ...
  it('creates education successfully', async () => {
    const req = {
      json: async () => ({ education_id: 'new-education-3', name: 'Test' })
    };
...
describe('PUT /educations/[id]', () => {
  ...
  it('returns 400 if id is missing', async () => {
    ...
    expect(res.body.error).toBe('Education ID is required');
  });
  ...
  it('returns 404 if education does not exist', async () => {
    supabase.maybeSingle.mockResolvedValueOnce({
      data: null,
      error: null
    });
    ...
    expect(res.body.error).toBe('Education data not found');
  });
  ...
  it('updates education successfully', async () => {
    supabase = createSupabaseMock({
      id: '123',
      education_id: 'new-education-123',
      title: '123',
      is_active: true,
      deleted_at: null
    });
...
describe('PATCH /educations/[id]', () => {
  ...
  it('returns 404 if education does not exist', async () => {
    ...
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('Archived education data not found');
  });
  ...
  it('restores education successfully if is_active is false', async () => {
    supabase = createSupabaseMock({
      id: '123',
      education_id: 'new-education-123',
      title: '123',
  ...
  it('restores education successfully if is_active is true', async () => {
    supabase = createSupabaseMock({
      id: '124',
      education_id: 'new-education-124',
      title: '124',
      is_active: true,
      deleted_at: null
    });
...
describe('DELETE /educations/[id]', () => {
  ...
  it('soft deletes education successfully if deleted_at is not null', async () => {
    supabase = createSupabaseMock({
      id: '123',
      education_id: 'new-education-123',
      title: 'test 123',
  ...
  it('soft deletes education successfully if is_active is true', async () => {
    supabase = createSupabaseMock({
      id: '123',
      education_id: 'new-education-123',
      title: 'test 123',
  ...
  it('soft deletes education successfully if is_active is false', async () => {
    supabase = createSupabaseMock({
      id: '124',
      education_id: 'new-education-124',
      title: 'test 124',
